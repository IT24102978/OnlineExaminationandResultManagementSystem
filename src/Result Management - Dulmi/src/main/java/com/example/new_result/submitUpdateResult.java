...
public class submitUpdateResult extends HttpServlet {
    ...
    private static final String RESULT_FILE_PATH = "D:/project/New_Result/src/main/webapp/data/results.txt";
...
//        String ca = request.getParameter("ca");
        ...
        try (BufferedReader reader = new BufferedReader(new FileReader(RESULT_FILE_PATH))) {
            ...
            e.printStackTrace();
            response.getWriter().println("Error reading the results file.");
            return;
        }
        ...
        if (recordUpdated) {
            try (BufferedWriter writer = new BufferedWriter(new FileWriter(RESULT_FILE_PATH))) {
                ...
                e.printStackTrace();
                response.getWriter().println("Error writing the results file.");
                return;
            }
