package online_exam_system.Service;
...
public class QuestionFileHandler {
    ...
    public static void initFilePath(String contextPath) {
        FILE_PATH = contextPath + "/question.txt";
        System.out.println("Initialized FILE_PATH: " + FILE_PATH);
        File file = new File(contextPath + "/question.txt");
        ...
        if (!file.exists()) {
            try {
                ...
                file.createNewFile();
                System.out.println("Created new question.txt at: " + contextPath);
            } catch (IOException e) {
